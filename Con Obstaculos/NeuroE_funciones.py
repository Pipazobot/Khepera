...
def aplicar_diferencia_a_velocidad(vel, angulo_error, modo='exponencial'):
...
def evitar_colision(sens, vel):
	ang_inventado = 100 

	if sens[0]>0:
		ang_inventado = -100

	vel_giro = aplicar_diferencia_a_velocidad(vel, ang_inventado, modo='exponencial')
	return vel_giro


# Principal
def Principal(bufffer_estados, target_pos, time, estado, t_alarma):

	# Extraer datos del buffer - Posición del robot movil
	...
	# Extraer sensores de distancia
	sensores = np.around(estado[6:9],3) #los 3 sensores frontales

	# Obetener el angulo de error
	angulo_error = angulo_de_error_con_signo(target_pos, pos_0, pos_2)
	#print('Angulo de error {} °'.format(angulo_error))

	# Activacion inicial de la alarma, si detecta un obstaculo
	if sum(sensores) > 0.4 and t_alarma==0:
		t_alarma = 15	# tiempo que estara encendida la alarma

	# Segun 't' calcular velocidad base
	Vel_ruedas = calcular_velocidad(time)

	# La accion que se debe hacer cuando la Alarma esta encendida
	modo = 'exponencial'
	if t_alarma > 0:
		print('=Alarma', end=' ')
		Vel_ruedas = evitar_colision(sensores, Vel_ruedas	)
		t_alarma += -1
		modo = 'lineal'

	Vel_ruedas = aplicar_diferencia_a_velocidad(Vel_ruedas, angulo_error, modo)
	Vel_ruedas = np.clip(Vel_ruedas, -7.5, +7.5)

	print('Vel_ruedas: ',Vel_ruedas)
	return Vel_ruedas 
